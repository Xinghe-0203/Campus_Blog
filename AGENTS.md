...
**Access:**
- Application: http://localhost:8825/api
- API Docs: http://localhost:8825/api/doc.html (Knife4j UI)
...
| `blog_draft_tag` | Draft-Tag relation | composite PK (draftId, tagId), 1NF normalization |
| `blog_report` | Reports | content reporting |
| `blog_circle_post` | Circle posts | alumni circle posts |
| `blog_circle_like` | Circle likes | alumni circle likes |
| `blog_circle_comment` | Circle comments | alumni circle comments |
| `blog_circle_repost` | Circle reposts | alumni circle reposts |
| `blog_media` | Media files | images/videos upload |
| `blog_post_media` | Post-Media relation | post media attachments |
| `blog_topic` | Topics | topic tags for posts |
| `blog_message` | Messages | private messages between users |
...
| `CaffeineCacheConfig.java` | Caffeine 缓存 + 限流配置 |

---

## Development Status

✅ **Completed**: User auth, article management, Spring Security + JWT, all entities/mappers/services, security hardening, entity validation, like/collect/comment functionality, login lock, JWT blacklist/refresh, XSS filter (Jsoup), concurrency fixes (fine-grained locks, DuplicateKeyException handling), follow system, notification system, trending system, draft auto-save, report management, circle posts, circle likes/comments/reposts, circle search, media upload, password change, user search, advanced post search, **circle visibility/permissions (like Weibo/Twitter)**, **frontend API documentation (complete)**, **内容审核**, **私信**, **密码找回**, **Rate Limiting**, **缓存策略修复**, **CirclePost逻辑删除统一**, **JSON TypeHandler**, **1NF规范化(blog_draft_tag)**, **外键约束参考SQL**

⏳ **Pending**: Frontend pages implementation, frontend-backend integration, Redis integration (distributed cache/rate limiting)
...
1. **阅读项目文档** - 修改前先阅读 `campus_blog.md`、`README.md`、`AGENTS.md`，了解项目架构和已有设计
